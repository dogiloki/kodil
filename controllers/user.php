<?php
	...
	if(isset($_POST)){
		...
		if($_GET['v']=='register'){
			...
			$debito=$_POST['debito'];
			$id=uniqid();
			mysqli_query($conexion,"INSERT INTO users VALUES ('".$id."','$name','$surname','$email','$birth','$phone','$sexo','$user','$password','$img','$account','0','1','1500',now(),'1','0')") or die ('error');
			mysqli_query($conexion,"INSERT INTO debito VALUES ('".uniqid()."','$account','$debito','-','07/23','0','0','9000','999999999')") or die ('error');
			mysqli_query($conexion,"INSERT INTO finanzas VALUES ('".uniqid()."','$account',0,0)") or die ('error');
			mysqli_query($conexion,"INSERT INTO movimientos VALUES ('".uniqid()."','KODIL','$account','cuenta','Bienvenido a Kodil',1500,now())") or die ('error');
			mysqli_query($conexion,"UPDATE finanzas SET ingresos=ingresos+1500 WHERE account='".$account."'");
			mysqli_query($conexion,"UPDATE digitos SET id_user='".$id."' WHERE digito='".$debito."'");
			echo "registrado";
			return "EXISTO";
		}
