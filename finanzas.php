...
if(!isset($_SESSION['kodil']['id'])){
	...
	<section class="content">
		<div class="option">
			...
			<section class="content-datos" id='content-info'>
				<?php
					$rs=mysqli_query($conexion,"SELECT * FROM finanzas WHERE account='".$_SESSION['kodil']['account']."'");
					while($row=mysqli_fetch_array($rs)){
						echo "<div class='datos' style='color: green'>INGRESOS: +".number_format($row['ingresos'])."</div>";
						echo "<div class='datos' style='color: red'>GASTOS: -".number_format($row['gastos'])."</div>";
					}
				?>
			</section>
			<section class="content-datos" id='content-info'>
				<strong>TODOS TUS MOVIMIENTOS</strong>
				<?php
						$rs_movi=mysqli_query($conexion,"SELECT * FROM movimientos WHERE desde='".$_SESSION['kodil']['account']."' OR para='".$_SESSION['kodil']['account']."' ORDER BY date DESC");
