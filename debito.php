...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/debito.css">
	...
	<section class="content">
		<div class="option">
			<nav class="menu">
				...
				<div class="texto" id="confi" onclick="cambio('confi')">CONFIGURACIÓN</div>
			</nav>
			<section class="content-datos" id='content-info' style="display: none;">
				<div class="actualizar">TARJETA DE DÉBITO</div>
				...
				<div class="content-info-info">
					<strong class="tilte-info">Número de tarjeta</strong>
					<?php
						$rs=mysqli_query($conexion,"SELECT * FROM debito WHERE account='".$_SESSION['kodil']['account']."'");
						while($row=mysqli_fetch_array($rs)){
							echo "<div class='info'>".$row['digitos']."</div>";
						}
					?>
				</div>
				<div class="content-info-info">
					<strong class="tilte-info">Fecha de vencimiento</strong>
					<?php
						$rs=mysqli_query($conexion,"SELECT * FROM debito WHERE account='".$_SESSION['kodil']['account']."'");
						while($row=mysqli_fetch_array($rs)){
							echo "<div class='info'>".$row['vencimiento']."</div>";
						}
					?>
				</div>
			</section>
			<section class="content-datos" id='content-movi' style="display: none;">
				<div class="actualizar">TARJETA DE DÉBITO</div>
					<?php
						$rs_movi=mysqli_query($conexion,"SELECT * FROM movimientos WHERE realizado='debito' AND (desde='".$_SESSION['kodil']['account']."' OR para='".$_SESSION['kodil']['account']."') ORDER BY date DESC");
						while($row_movi=mysqli_fetch_array($rs_movi)){
							...
							<div class='content-info-movi' style="cursor: pointer;" onclick="modal('<?php echo $row_movi['id']; ?>')">
								<div class='movi'><?php echo utf8_encode($row_movi['tipo_operacion']); ?></div>
									...
									<div class="info_movi">
										...
										<?php
											if($row_movi['desde']==$_SESSION['kodil']['account']){
												echo "<div class='text-movi' style='color: red;'>Cantidad: -".number_format($row_movi['cantidad'])."</div>";
											}else{
												echo "<div class='text-movi' style='color: green;'>Cantidad: +".number_format($row_movi['cantidad'])."</div>";
											}
										?>
			...
			<section class="content-datos" id='content-confi' style="display: none;">
				<div class="actualizar">TARJETA DE DÉBITO</div>
				<div class='content-info-confi'>
					<a class="text-confi">
						<?php
							$rs_tarjeta=mysqli_query($conexion,"SELECT * FROM debito WHERE account='".$_SESSION['kodil']['account']."'");
							while($row_tarjeta=mysqli_fetch_array($rs_tarjeta)){
								//Cancelar
								if($row_tarjeta['cancelada']=='0'){
									?>
									<input type="checkbox" style="margin-right: 7px;" onchange="cancelar('debito')" id='cancelar' class='confi'>Cancelar tarjeta
									<?php
								}else{
									?>
									<input type="checkbox" style="margin-right: 7px;" onchange="cancelar('debito')" id='cancelar' class='confi' checked>Cancelar tarjeta
									<?php
								}
							}
						?>
					...
					<a class="text-confi">
						<?php
							...
							while($row_tarjeta=mysqli_fetch_array($rs_tarjeta)){
								//Limite de retiro
								?>
								Limite de retiro<input type="number" value="<?php echo $row_tarjeta['retiro']; ?>" style="margin-left: 7px;" id='caja'>
								<?php
							}
						?>
					</a>
					...
					<a class="text-confi">
						<?php
							...
							while($row_tarjeta=mysqli_fetch_array($rs_tarjeta)){
								//Limite de retiro
								?>
								Limite de compra<input type="number" value="<?php echo $row_tarjeta['compra']; ?>" style="margin-left: 7px;" id='caja'>
								<?php
							}
						?>
					</a>
